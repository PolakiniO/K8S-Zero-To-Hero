...
```bash
polakinio@Polakinio:~/Projects/k8s/week2$ kubectl delete deployment web --ignore-not-found
polakinio@Polakinio:~/Projects/k8s/week2$ kubectl delete rs -l app=web --ignore-not-found
No resources found
polakinio@Polakinio:~/Projects/k8s/week2$ kubectl delete pod -l app=web --ignore-not-found
...
```bash
polakinio@Polakinio:~/Projects/k8s/week2$touch lab7-deploy-good.yaml
polakinio@Polakinio:~/Projects/k8s/week2$ kubectl apply -f lab7-deploy-good.yaml
deployment.apps/web created

polakinio@Polakinio:~/Projects/k8s/week2$ kubectl get pods -w
...
polakinio@Polakinio:~/Projects/k8s/week2$ kubectl delete pod curlpod
pod"curlpod" deleted
...
```bash
polakinio@Polakinio:~/Projects/k8s/week2$touch lab7-deploy-broken.yaml
polakinio@Polakinio:~/Projects/k8s/week2$ kubectl apply -f lab7-deploy-broken.yaml
deployment.apps/web configured

polakinio@Polakinio:~/Projects/k8s/week2$ kubectl rollout status deployment/web
...
polakinio@Polakinio:~/Projects/k8s/week2$ kubectl desctibe pod web-5cc4949757-fp4wh
error: unknowncommand"desctibe"for"kubectl"

Did you mean this?
        describe

polakinio@Polakinio:~/Projects/k8s/week2$ kubectl describe pod web-5cc4949757-fp4wh
...
...
polakinio@Polakinio:~/Projects/k8s/week2$ kubectl get events --sort-by=.metadata.creationTimestamp
...
...
```bash
polakinio@Polakinio:~/Projects/k8s/week2$ kubectl rollouthistory deployment/web
deployment.apps/web
...
```bash
polakinio@Polakinio:~/Projects/k8s/week2$ kubectl rollout undo deployment/web
